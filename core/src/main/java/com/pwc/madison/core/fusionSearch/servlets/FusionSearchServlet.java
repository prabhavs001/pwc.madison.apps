...
@Component(service = Servlet.class, name = "PwC Viewpoint Fusion Search Servlet", property = {
		org.osgi.framework.Constants.SERVICE_DESCRIPTION + "=Lucidworks Fusion Search Servlet for PwC Viewpoint",
		"sling.servlet.paths=/bin/pwc-madison/vp-search", "sling.servlet.methods=" + HttpConstants.METHOD_GET })
public class FusionSearchServlet extends SlingSafeMethodsServlet {
	...
	protected void doGet(@Nonnull final SlingHttpServletRequest request,
		...
		try {
			...
			final WebResource webResource = client.resource(fusionSearchConfigurationService.getSearchEndpoint()
	...
	private void sendErrorResponse(final SlingHttpServletResponse response) throws IOException {
		...
		response.getWriter().write("Error in Viewpoint Search Servlet");
	}
