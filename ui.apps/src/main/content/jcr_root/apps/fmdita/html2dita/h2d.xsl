...
<xsl:stylesheet version="3.0" 
        ...
        xmlns:smap="http://www.w3.org/2005/xpath-functions/map"
                extension-element-prefixes="saxon map smap">

<!--  <xsl:character-map name="a">
    <xsl:output-character character="&#160;" string="&amp;#160;" />
    <xsl:output-character character="â€œ" string="&amp;quot;" />
...
<xsl:variable name="elemsList" select="tokenize($elems, ',')" />
<xsl:param name="condMap"/>
<xsl:param name="style2attrMap"/>
<xsl:param name="referType" select="'conref'"/>

 <!-- pwc - changing the dtd to pwc topic -->
<xsl:variable name="systemid">
    <xsl:choose>
        ...
        <xsl:otherwise>../dtd/pwc-topic.dtd</xsl:otherwise>
    </xsl:choose>
</xsl:variable>
<xsl:variable name="publicid">
    <xsl:choose>
        ...
        <xsl:otherwise>-//OASIS//DTD DITA PWC Topic//EN</xsl:otherwise>
    </xsl:choose>
</xsl:variable>
<!--<xsl:output method="xml" indent="yes" encoding="us-ascii" doctype-system="{$systemid}" doctype-public="{$publicid}"/>-->
  
<xsl:include href="/apps/fmdita/html2dita/h2d_extended.xsl"/>

<!--<xsl:output indent="yes" encoding="us-ascii" method="xml" doctype-system="technicalContent/dtd/topic.dtd" 
     doctype-public="-//OASIS//DTD DITA Topic//EN" /> -->
     
     <!--  <xsl:output indent="yes" use-character-maps="a" method="xml" doctype-system="technicalContent/dtd/topic.dtd" 
     doctype-public="-//OASIS//DTD DITA Topic//EN" />  -->
...
<!-- pwc - changing the doctype topic to pwc topic -->
<xsl:template match="*[local-name()='html']">
    <xsl:text disable-output-escaping='yes'>&lt;!DOCTYPE pwc-topic </xsl:text>
    <!--<xsl:value-of select="$infotype"/>-->
    ...
    <xsl:choose>
        ...
        <xsl:when test="$infotype='concept'">
            <xsl:call-template name="gen-concept"/>
        </xsl:when>
        <xsl:when test="$infotype='task'">
            <xsl:call-template name="gen-task"/>
        </xsl:when>
        <xsl:when test="$infotype='reference'">
            <xsl:call-template name="gen-reference"/>
        </xsl:when>
        <xsl:otherwise>
            <xsl:call-template name="gen-topic"/>
        </xsl:otherwise>
...
<xsl:template name="gen-topic">
  <pwc-topic xml:lang="{$default-lang}">
...
<xsl:template name="gen-inset-topic">
  ...
  <topic xml:lang="{$default-lang}">
    ...
    <body>
      <bodydiv>
        <xsl:attribute name="id" select="$sectionId"/>
        <xsl:apply-templates/>
      </bodydiv>
    </body>
    <xsl:call-template name="genrellinks"/>
...
<xsl:template match="h1" priority="5">
  <xsl:choose>
    <xsl:when test="$main-head-level='h1' and not(preceding::h1)"/>
...
<xsl:template name="convert-heading-to-section">
  <section>
    <title><xsl:apply-templates select="@class|@data-conditional"/><xsl:apply-templates select="*|comment()|text()"/></title>
    <xsl:apply-templates select="(following-sibling::*|following-sibling::text()|following-sibling::comment())[1]" mode="add-content-to-section"/>
  </section>
</xsl:template>
...
The heading has been placed in a required-cleanup element.</xsl:with-param>
      ...
      <section>
        <required-cleanup>
          <title><xsl:apply-templates select="@class|@data-conditional"/><xsl:apply-templates select="*|text()|comment()"/></title>
          <xsl:apply-templates select="(following-sibling::*|following-sibling::text()|following-sibling::comment())[1]" mode="add-content-to-section"/>
        </required-cleanup>
      </section>
    </xsl:otherwise>
...
<!-- pwc - picking the title from the title tag instead of the heading -->
...
<xsl:template match="@class">
  ...
  <xsl:variable name="attrVal" select="map:get($style2attrMap,string($class))" />
  <xsl:if test="$attrVal">
    <xsl:variable name="attr" select="substring-before(string($attrVal), '=')"/>
    <xsl:variable name="val" select="substring-after(string($attrVal), '=')"/>
    <xsl:attribute name="{$attr}"><xsl:value-of select="$val"/></xsl:attribute>
  </xsl:if>
</xsl:template>

<xsl:template match="@data-conditional">
  <xsl:variable name="ditaCondMap" select="smap:merge(smap:entry('',''))"/>
  <xsl:variable name="conditionsInit"><xsl:value-of select="."/></xsl:variable>
  <xsl:variable name="conditions" select="concat($conditionsInit,',')"/>
  <xsl:call-template name="genCond">
     <xsl:with-param name="conditions" select="$conditions"/>
     <xsl:with-param name="ditaCondMap" select="$ditaCondMap"/>
  </xsl:call-template>
</xsl:template>

<xsl:template name="genCond">
  <xsl:param name="conditions"/>
  <xsl:param name="ditaCondMap"/>
  <xsl:param name="ditaCondMapVal"/>
  <xsl:variable name="cond" select="substring-before($conditions, ',')"/>
  <xsl:variable name="condType" select="map:get($condMap,string($cond))" />  
  <xsl:if test="not($cond='')">
    <xsl:choose>
      <xsl:when test="$condType">
        <xsl:variable name="oldAttr" select="smap:get($ditaCondMap,string($condType))" />
        <xsl:variable name="newAttr" select="concat($oldAttr, ' ',  $cond)" />
        <xsl:attribute name="{$condType}"><xsl:value-of select="$newAttr"/></xsl:attribute>
        <xsl:call-template name="genCond">
           <xsl:with-param name="conditions" select="substring-after($conditions, ',')"/>
           <xsl:with-param name="ditaCondMap" select="smap:put($ditaCondMap,string($condType), string($newAttr))"/>
        </xsl:call-template>
      </xsl:when>
      <xsl:otherwise>
        <xsl:variable name="oldAttr" select="smap:get($ditaCondMap,'otherprops')" />
        <xsl:variable name="newAttr" select="concat($oldAttr, ' ',  $cond)" />
        <xsl:attribute name="otherprops"><xsl:value-of select="$newAttr"/></xsl:attribute>
        <xsl:call-template name="genCond">
           <xsl:with-param name="conditions" select="substring-after($conditions, ',')"/>
           <xsl:with-param name="ditaCondMap" select="smap:put($ditaCondMap,'otherprops', string($newAttr))"/>
        </xsl:call-template>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:if>
...
<xsl:template match="code">
  <codeph><xsl:apply-templates select="@class|@data-conditional|*|text()|comment()"/></codeph>
</xsl:template>

<xsl:template match="var">
  <varname><xsl:apply-templates select="@class|@data-conditional"/><xsl:value-of select="."/></varname>
</xsl:template>

<xsl:template match="samp">
  <systemoutput><xsl:apply-templates select="@class|@data-conditional|*|text()|comment()"/></systemoutput>
</xsl:template>

<xsl:template match="kbd">
  <userinput><xsl:apply-templates select="@class|@data-conditional|*|text()|comment()"/></userinput>
</xsl:template>


<xsl:template match="em">
  <i><xsl:apply-templates select="@class|@data-conditional|*|text()|comment()"/></i>
</xsl:template>

<xsl:template match="strong">
  <b><xsl:apply-templates select="@class|@data-conditional|*|text()|comment()"/></b>
</xsl:template>

<xsl:template match="blockquote">
  <lq><xsl:apply-templates select="@class|@data-conditional|*|text()|comment()"/></lq>
</xsl:template>

<!-- <lq> in <lq> is invalid in DITA, so make it valid (though it is a bit strange) -->
<xsl:template match="blockquote/blockquote">
  <p><lq><xsl:apply-templates select="@class|@data-conditional|*|text()|comment()"/></lq></p>
</xsl:template>

<xsl:template match="pre" priority="3">
  <codeblock><xsl:apply-templates select="@class|@data-conditional|*|text()|comment()"/></codeblock>
</xsl:template>
    ...
    <xsl:otherwise>
      <xsl:choose>
        <xsl:when test="@href and parent::body">
          <p><pwc-xref> <!-- pwc - using pwc-xref instead of xref -->
            <xsl:call-template name="genlinkattrs"/>
            <xsl:apply-templates select="@class|@data-conditional"/>
            <xsl:apply-templates select="*|text()|comment()"/>
          </pwc-xref></p>
        ...
        <xsl:when test="@href">
          <pwc-xref> <!-- pwc - using pwc-xref instead of xref -->
            <xsl:call-template name="genlinkattrs"/>
            <xsl:apply-templates select="@class|@data-conditional"/>
            <xsl:apply-templates select="*|text()|comment()"/>
          </pwc-xref>
        ...
        <xsl:when test="parent::body and text()">
          <p><xsl:apply-templates select="@class|@data-conditional"/><xsl:apply-templates select="*|text()|comment()"/></p>
        </xsl:when>
        <xsl:otherwise>
          <xsl:apply-templates select="*|text()|comment()"/>
        </xsl:otherwise>
...
<xsl:template match="table/caption">
  <title>
		<xsl:if test="@id"><xsl:attribute name="id"><xsl:value-of select="@id"/></xsl:attribute></xsl:if>
		<xsl:apply-templates select="@class|@data-conditional|*|text()|comment()"/>
	</title>
</xsl:template>
...
<row>
    ...
    <xsl:apply-templates select="@class|@data-conditional"/>
    <xsl:attribute name="rowsep"><xsl:value-of select="'1'"/></xsl:attribute>
    <xsl:apply-templates/>
</row>
</xsl:template>
...
<entry>
  ...
  <xsl:if test="@colspan">  <!-- Allow entries to span columns -->
    <xsl:variable name="current-cell-temp"><xsl:call-template name="current-cell-position"/></xsl:variable>
    <xsl:variable name="current-cell" select="if ($current-cell-temp='')
                 then 1
                 else
                   $current-cell-temp"/>
...
<xsl:template match="td/p|th/p">
  <xsl:choose>
    <xsl:when test="following-sibling::*|preceding-sibling::*">
      <p><xsl:apply-templates select="@class|@data-conditional|*|text()|comment()"/></p>
    </xsl:when>
...
<!-- pwc - basic formatting for inline styles -->
<xsl:template match="span[@style]">
    <xsl:variable name="span-style"><xsl:value-of select="@style"/></xsl:variable>
    <xsl:choose>
        <xsl:when test="contains($span-style,'font-weight:bold') or contains($span-style,'font-weight :bold') or
            contains($span-style,'font-weight: bold') or
            contains($span-style,'font-weight : bold')">
            <b>
                <xsl:apply-templates select="*|text()|comment()"/>
            </b>
        </xsl:when>
        <xsl:when test="contains($span-style,'font-style:italic') or contains($span-style,'font-style :italic') or
                contains($span-style,'font-style: italic') or
                contains($span-style,'font-style : italic')">
                <i>
                    <xsl:apply-templates select="*|text()|comment()"/>
                </i>
        </xsl:when>
        <xsl:when test="contains($span-style,'text-decoration: underline') or contains($span-style,'text-decoration :underline') or
                contains($span-style,'text-decoration: underline') or
                contains($span-style,'text-decoration:underline') or
                contains($span-style,'text-decoration : underline')">
                <u>
                    <xsl:apply-templates select="*|text()|comment()"/>
                </u>
        </xsl:when>
        <xsl:when test="contains($span-style,'font-family:Courier') or contains($span-style,'font-family :Courier') or
                contains($span-style,'font-family: Courier') or
                contains($span-style,'font-family : Courier')">
                <tt>
                    <xsl:apply-templates select="*|text()|comment()"/>
                </tt>    
        </xsl:when>
        <xsl:when test="contains($span-style,'font-weight:normal') or contains($span-style,'font-weight :normal') or
                contains($span-style,'font-weight: normal') or
                contains($span-style,'font-weight : normal')">
                <normal>
                    <xsl:apply-templates select="*|text()|comment()"/>
                </normal>    
        </xsl:when>
...
There is a comment next to the phrase with the span's class value.</xsl:with-param>
      ...
      <ph><xsl:apply-templates select="@class|@data-conditional"/><xsl:apply-templates select="*|text()|comment()"/></ph>
    </xsl:otherwise>
  </xsl:choose>
</xsl:template>
...
<dlentry>
 <dt><xsl:apply-templates select="@class|@data-conditional|*|text()|comment()"/></dt>
...
<xsl:template match="body/i|body/div/i" priority="4">
  <p><i><xsl:apply-templates select="@class|@data-conditional|*|text()|comment()"/></i></p>
</xsl:template>
<xsl:template match="body/b|body/div/b" priority="4">
  <p><b><xsl:apply-templates select="@class|@data-conditional|*|text()|comment()"/></b></p>
</xsl:template>
<xsl:template match="body/u|body/div/u" priority="4">
  <p><u><xsl:apply-templates select="@class|@data-conditional|*|text()|comment()"/></u></p>
</xsl:template>
...
<xsl:template match="msub|msup|mn|mo|mi|mn|mfrac|mfenced|mrow|mstyle">
	<xsl:element name="m:{local-name()}" namespace="http://www.w3.org/1998/Math/MathML">
		<xsl:copy-of select="@*"/>
		<xsl:apply-templates/>
	</xsl:element>
...
<xsl:template match="*">
  <xsl:choose>
    <xsl:when test="index-of($elemsList, name(.))">
          <xsl:copy>
            <xsl:apply-templates select="@class|@data-conditional"/>
            <xsl:apply-templates/>
          </xsl:copy>
...
<xsl:template match="div[@data-conref]">
  <xsl:choose>
    <xsl:when test="((parent::body or parent::div[parent::body]) and ($referType = 'conref')) or ($referType = 'xref')">
      <xsl:variable name="href" select="tokenize(@data-conref, '/')[last()]"/>
      <xsl:variable name="hrefDita" select="concat('INSET_FILE_',substring-before($href, '.'),'.dita')"/>
      <xsl:variable name="topicId" select="concat('topic',generate-id(.))"/>
      <xsl:variable name="sectionId" select="concat('section',generate-id(.))"/>
      <xsl:choose>
        <xsl:when test="($referType = 'conref')">
          <bodydiv conref="{$hrefDita}#{$topicId}/{$sectionId}"/>
        </xsl:when>
        <xsl:otherwise>
          <p><xref href="{$hrefDita}"><xsl:value-of select='$hrefDita'/></xref></p>
        </xsl:otherwise>
      </xsl:choose>
      <xsl:result-document href="{$hrefDita}">
          ...
          <xsl:value-of select="$systemid" />
          <xsl:text disable-output-escaping='yes'>"></xsl:text>
          <xsl:call-template name="gen-inset-topic">
